...
class HiwinMoveitAction
{
  ...
  public:
    ...
    HiwinMoveitAction(std::string name) : //nh_: action 所在ros節點的節點名稱
                                          //name: 為action的名字，server&client的依據 參考controllers.yaml參數來寫 /follow_joint_trajectory
                                          //boost為固定用法
                                          as_(nh_, "hiwin_6dot/follow_joint_trajectory", boost::bind(&HiwinMoveitAction::executeCB, this), false),
                                          action_name_(name)
    {
        cout << "start action\n";
        //as_.registerGoalCallback(boost::bind(&HiwinMoveitAction::executeCB, this, _1));
        as_.registerPreemptCallback(boost::bind(&HiwinMoveitAction::preemptCB, this));

        as_.start(); //啟動action server
        executeCB();
        ROS_WARN("start action server by hiwin_driver");
        joint_msg_pub = nh_.advertise<std_msgs::String>("/pub_string", 1000);
        s_msg.data = "start action server by hiwin_driver\n";
        joint_msg_pub.publish(s_msg);
    }
    ...
    void executeCB()//const control_msgs::FollowJointTrajectoryGoalConstPtr &msg)
    {
        ...
        s_msg.data = "in execution CB func\n";
