...
class HiwinMoveitAction
{
  ...
  public:
    ...
    //void executeCB(const control_msgs::FollowJointTrajectoryGoalConstPtr msg)
    {
        ...
        int goal_index = 0;
        int n_joints = goal->trajectory.joint_names.size(); //4
        ...
        while (n.ok() && as_.isActive())
        {
            ...
            else
            {
                ...
                double pos[4]; //set radian -> degree
                trajectory_msgs::JointTrajectoryPoint pos_point;
                printf("Goal index: %d\n", goal_index);
                for (unsigned int i = 0; i < n_joints; i++)
                {
                    printf("original_pos[%d]: %f\n", i, hiwin_ref.position[i]);
                    if(i==2) pos[i] =hiwin_ref.position[i]*1000;//-50;
                    else pos[i] = hiwin_ref.position[i] * 57.29577951;
                ...
                // Publish feedback
                double return_pos[4] = {0.0, 0.0, 0.0, 0.0};
                if (device_id >= 0)
                {
                    ...
                    else
                    {
                        ...
                        for (unsigned int i = 0; i < n_joints; i++)
                        {
                            ...
                            if (return_pos[i] != 0.0)
                            {
                                if(i!=2) return_pos[i] = return_pos[i] / 57.29577951;
                            }
...
int main(int argc, char **argv)
{
    ...
    HiwinMoveitAction HiwinMoveitAction("hiwin_4dot/follow_joint_trajectory");
	...
	ros::Publisher joint_pub = nh_.advertise<sensor_msgs::JointState>("/move_group/hiwin_4dot_moveit_joint_states", 1);
	sensor_msgs::JointState hiwin_return;
	double return_pos[4] = {0.0, 0.0, 0.0, 0.0};
	hiwin_return.name.push_back("j_0001");
	hiwin_return.name.push_back("j_0102");
	hiwin_return.name.push_back("j_02sq");
	hiwin_return.name.push_back("j_sq03");
